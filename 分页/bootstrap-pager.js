...
(function ($, window, document, undefined) {
    ...
    var BasePage = function (element, options) {
        ...
        console.log(" constract options:",this.options);
        this.init();
    }
    ...
    BasePage.prototype.template = {
        ...
        li_next_a: '<a href="javascript:void(0)" class="next" style="border:0px;" aria-controls="{0}" data-dt-idx="{1}" tabindex="{1}">下一页</a>'
    }
    ...
    BasePage.prototype.refresh=function (pageInfo) {
        ...
        var url=pageInfo.url||this.options.url;
        var success=pageInfo.success||this.options.success;
        var options = {
                maxPage:maxPage,
                pageSize:pageSize,
                pageNum:pageNum,
    ...
    BasePage.prototype.bindEvent = function () {
        ...
        this.$element.find("li a").not(".disabled").not(".next").not(".previous").each(function (index, item) {
            $(item).click(function () {
                ...
                pageInfo = $.extend(true, {}, this.options, pageInfo);
                self.$element.trigger("pageChanged", pageInfo);
                self.$element.data("page").refresh(pageInfo);
    ...
    BasePage.prototype.load = function(){
    	...
    	url+="pagesize="+ options.pageSize+"&pageindex="+pageNum;
    	
    	$.ajax({
			url:url,
			method:"get", 
			success:function(data){
				self.options.count = data.total;
				self.drawPage(self.options.count, self.options.pageNum || 1);
				options.success(data);
				var width = $(".pagination").width();
				console.log("width:",width);
				var p_width =  $(".pagination").parent().width();
				console.log("p_width:",p_width);

				var left =p_width-width;
				$(".pagination").parent().css({
					"visibility":"visible"
				});
				$(".pagination").css({
					"padding-left":left*0.5+"px",
					"bottom": "10px"
				});
				$("#pagination_next").css({
					"padding-left":"0px",
